...
using PosClient.Desktop.Features.Inventory.Products.Editor.Tabs;
using PosClient.Desktop.Features.Inventory.Products.List;
...
namespace PosClient.Desktop.Features.Inventory.Products.Editor
{
    public partial class ProductEditorViewModel : ObservableObject, INavigationAware
    {
        ...
        [ObservableProperty]
        private ProductDetails? _currentProduct;
        ...
        {
            if (_productStateService.SelectedProductId == null)
            {
                _navigationService.Navigate(typeof(ProductListPage));
            }
            else if(_productStateService.IsEditingNewProduct)
            {
                await InitializeNew();
            }
            else
            {
                await InitializeEdit(_productStateService.SelectedProductId.Value);
            }
        }
        ...
        {
            IsNew = true;
            CurrentProduct = new ProductDetails
            {
                IsActive = true,
                BasePrice = 0
            };
        ...
        public async Task ToggleVariantStatus(ProductVariant variant)
        {
            ...
            if (variant.IsActive)
            {
                ...
                if (confirm)
                {
                    ...
                    if (result.IsSuccess)
                    {
                        ...
                        _notificationService.ShowSuccess("Product variant deactivated successfully.");
                    }
                }
            }
            else
            {
                ...
                //        _notificationService.ShowSuccess("Product variant activated successfully.");
                //    }
                //}
        ...
        public async Task OpenGenerator()
        {
            ...
            if (result == Wpf.Ui.Controls.ContentDialogResult.Primary)
            {
                ...
                {
                    await Save();
                    _notificationService.ShowSuccess($"{created} variants created.");
                }
                else
                {
        ...
        public async Task SetPrimaryImage(ProductImage image)
        {
            ...
            if (confirm)
            {
                ...
                if (result.IsSuccess)
                {
                    _notificationService.ShowSuccess("Image is set as primary");
                    await InitializeEdit(CurrentProduct.Id);
                }
            }
        }
        ...
        public async Task Save()
        {
            ...
            if (IsNew)
            {
                ...
                if (result.IsSuccess)
                {
                    _notificationService.ShowSuccess("Product saved.");
                    if (result.Data != null)
            ...
            else
            {
                ...
                if (result.IsSuccess)
                {
                    _notificationService.ShowSuccess("Product updated");
                    await InitializeEdit(CurrentProduct.Id);
                }
            }
